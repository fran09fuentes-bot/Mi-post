...
export default function ArqueoCaja() {
  const STORAGE_GASTOS_KEY = 'pos_gst';
  const STORAGE_VENTAS_KEY = 'pos_vts';
  ...
  const handleEliminarGasto = (idGasto, conceptoGasto) => {
    if (window.confirm(`¿Quieres eliminar "${conceptoGasto}"?`)) {
      const listaFiltrada = gastos.filter((item) => (item.id ? item.id !== idGasto : item.concepto !== conceptoGasto));
      setGastos(listaFiltrada);
      localStorage.setItem(STORAGE_GASTOS_KEY, JSON.stringify(listaFiltrada));
    }
  };

  const totalVentasBrutas = ventasDelDia.reduce((acc, v) => acc + Number(v.ingreso || v.total || v.monto || 0), 0);
            ...
            {gastos.map((item, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => handleEliminarGasto(item.id || idx, item.concepto)}
